...
def process_data(
    ...
    # 1. 读取、初步统计、清洗
    # -------------------
    # 使用latin-1编码读取assist2009
    df = pd.read_csv(read_file, encoding="latin-1",  dtype=str)
    ...
    df = df.dropna(subset=["user_id", "problem_id", "correct", "skill_id", "order_id"])
    # df = df[df["correct"].isin([0, 1])]  # 确保correct字段只有0和1
    ...
    print(f"Length filtering {min_seq_len}: Before filtering: {len(student_seq_lens)}, After filtering: {len(valid_students)}, Removed {len(student_seq_lens) - len(valid_students)} students")
    ...
    # -------------------
    if time_gap_weeks > 0:
        raise ValueError(f"No time information")

    # 确保测试标记没丢
    assert processed_df["is_test"].sum() > 0, "Error: Test set marking lost!"
    ...
    for uid in processed_students:
        ...
        # 原始 index 排序
        block = block.sort_values(by=["index"])
