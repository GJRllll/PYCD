...
def load_nips_data(primary_data_path, meta_data_dir, task_name):
    ...
    Data source: https://competitions.codalab.org/competitions/25449 
    Document: https://arxiv.org/abs/2007.12061

    Args:
        primary_data_path (str): Path to primary data
        meta_data_dir (str): Directory of meta data
        task_name (str): Task name, e.g. task_1_2 or task_3_4
    ...
    print(f"Primary data length: {len(df_primary)}")
    ...
    print(f"Merged data length: {len(df_merge)}")
    print("Data loading completed")
    print(f"Student number: {df_merge['UserId'].unique().size}")
    print(f"Question number: {df_merge['QuestionId'].unique().size}")
    
    kcs = []
    for item in df_merge['SubjectId_level3'].values:
        if isinstance(item, set):
            kcs.extend(item)
    print(f"Concept number: {len(set(kcs))}")
...
def process_data(
    ...
    # -------------------
    if mode == 1:
        df2 = df.sort_values(by=["UserId", "tmp_index"])
        processed_df = df2.drop_duplicates(
            subset=["UserId", "QuestionId"],
            keep="first",
            ignore_index=True
        )
    elif mode in [2, 3]:
    ...
    print(
        f"Training set accuracy: {processed_df[~processed_df['is_test']]['IsCorrect'].mean():.4f}, "
        f"Test set accuracy: {processed_df[processed_df['is_test']]['IsCorrect'].mean():.4f}"
    )
    ...
    for uid in processed_students:
        ...
        # 按时间戳 + 原始 index 排序
        block = block.sort_values(by=["answer_timestamp", "tmp_index"])
        seq_len = len(block)
