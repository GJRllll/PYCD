...
Q_FILE = "../data/frcsub/q.txt"
KEYS = ["user_id", "skill_id", "problem_id"]

def read_data_from_txt(frcsub_file, write_file, seed=42, test_ratio=0.2):
    """
    Processing method: Keep the original answer order, perform stratified sampling of the test set
    
    Args:
        frcsub_file (str): Input Frcsub data file path
    ...
    # 读取数据
    frcsub_data = pd.read_csv(frcsub_file, sep='\t', header=None)
    q_data = pd.read_csv(Q_FILE, sep='\t', header=None)
    
    # 转换为DataFrame格式，保持原始顺序
    rows = []
    for student_id in range(len(frcsub_data)):
        # 保持每个学生的问题顺序
        for idx in range(frcsub_data.shape[1]):
            # 提取技能ID
            ...
            rows.append({
                ...
                'problem_id': str(idx),
                'skill_id': skill,
                'correct': str(frcsub_data.iloc[student_id, idx]),
