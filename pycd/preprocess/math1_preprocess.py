...
Q_FILE = "../data/math1/q.txt"
KEYS = ["user_id", "skill_id", "problem_id"]

def read_data_from_txt(math1_file, write_file, seed=42, test_ratio=0.2):
    """
    Processing method: Keep the original answer order, perform stratified sampling of the test set
    
    Args:
        math1_file (str): Input math data file path
    ...
    # 读取数据
    math1_data = pd.read_csv(math1_file, sep='\t', header=None)
    q_data = pd.read_csv(Q_FILE, sep='\t', header=None)
    
    # 排除math1_data最后5列，q_data最后5行(主观题)
    math1_data = math1_data.iloc[:, :-5]
    q_data = q_data.iloc[:-5, :] 
    
    # 转换为DataFrame格式，保持原始顺序
    rows = []
    for student_id in range(len(math1_data)):
        # 保持每个学生的问题顺序
        for idx in range(math1_data.shape[1]):
            # 提取技能ID
            ...
            rows.append({
                ...
                'problem_id': str(idx),
                'skill_id': skill,
                'correct': str(math1_data.iloc[student_id, idx]),
